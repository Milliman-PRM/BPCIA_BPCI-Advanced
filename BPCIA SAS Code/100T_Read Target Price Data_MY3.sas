...
%macro read_in(in);
	...
	* overwriting EPI_CAT so it can map;
	data TP_Components_2;
		set TP_Components_1 (rename=(EPI_CAT=EPI_CAT_orig));
		if EPI_CAT_orig ne 'All';
		length EPI_CAT_adj $100;
		format EPI_TYPE $3.;
		EPI_TYPE = substr(EPI_CAT_orig,1,2);
		EPI_CAT = substr(EPI_CAT_orig,4,length(EPI_CAT_orig)-3);
		EPI_CAT_adj=EPI_CAT;
		...
		if EPI_CAT_adj='Transcathether aortic valve replacement' then EPI_CAT_adj='Endovascular Cardiac Valve Replacement';
	...
	* map on short episode names;
	proc sql;
	  ...
	  left join ref.bpcia_clinical_episode_names_MY3 as b
	...
	* create numeric variables from text fields;
	data tempdir.TP_Components;
		length EPI_INDEX $132;
		set TP_Components_3 (rename= (EPI_SPEND=EPI_SPEND_o AMT=AMT_o ACH_EFF=ACH_EFF_o SBS=SBS_o 
									PCMA=PCMA_o PAT_Adj=PAT_Adj_o PAT=PAT_o HBP=HBP_o PGP_EFF=PGP_EFF_o 
		...
		format EPI_SPEND AMT ACH_EFF SBS PCMA PAT_Adj PAT HBP PGP_EFF PGP_OFFSET PGP_OFFSET_ADJ PGP_ACH_PCMA CASE_MIX PGP_ACH_BNCHMRK TARGET_PRICE PAYMENT_RATIO TARGET_PRICE_REAL best12.;
		...
		PAT_Adj = compress(tranwrd(tranwrd(tranwrd(PAT_Adj_o, "$",""),",","")," ",""));
		...
		drop EPI_SPEND_o AMT_o ACH_EFF_o SBS_o PCMA_o PAT_Adj_o PAT_o HBP_o PGP_EFF_o
