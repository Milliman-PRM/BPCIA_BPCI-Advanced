...
Purpose is to identify transfer episodes that we believe CMS incorrectly assigned.
************************************************************
*/
...
%if &label. ^= ybase %then %do;
	...
	*Stack episode exclusion files created from CMS exclusions and all Milliman exclusion files;
	...
	quit;
	proc sql;
		create table epi_&label._&bpid1._&bpid2. as
		select a.*
		from epi_&label._&bpid1._&bpid2. as a
		where a.EPI_ID_MILLIMAN not in (select distinct EPI_ID_MILLIMAN from excl_trans_&bpid1._&bpid2.);
	quit;
	proc sql;
		create table epi as
